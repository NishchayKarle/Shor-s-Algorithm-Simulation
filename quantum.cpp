...
char Register::measure(unsigned int qubit)
{
	...
	*/
	...
	for (state_map::iterator i = states.begin(); i != states.end(); ++i)
	{
		s = i->first;
		if (s[qubit] == '0')
			zero_prob += probability(s);
	}

	state_map m;
	char v;
	amp p;
	if (get_rand() < zero_prob)
	{
		v = '0';
		p = sqrt(zero_prob);
	}
...
void Register::apply_gate(Unitary u, vec_int qubits)
{
	...
	*/
	...
	if (u.dimension != (unsigned int)(1 << qubits.size()))
	{ // 1 << qubits.size is pow(2, qubits.size())
...
void Register::ControlledNot(unsigned int control_qubit, unsigned int target_qubit)
{
	...
	*/
	vec_int v;
	v.push_back(control_qubit);
	v.push_back(target_qubit);
